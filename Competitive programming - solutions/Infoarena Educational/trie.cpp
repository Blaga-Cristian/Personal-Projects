...
bool testCase = false;
const ll N = 1e4 + 9, LG = 31, A = 26, Mod = 1e9 + 7;
...
struct node
{
    ...
    {
        cnt = 0;
        FOR(i, 0, A - 1)sons[i] = 0;
    }
};
node *t = new node();

void Add(node* nod = t, int p = 0)
{
    nod -> cnt ++;
    if(p == s.size())
    {
    ...
    if(!nod -> sons[s[p] - 'a'])
        nod -> sons[s[p] - 'a'] = new node();
    Add(nod -> sons[s[p] - 'a'], p + 1);
}

void Del(node* nod = t, int p = 0)
{
    nod -> cnt --;
    if(p == s.size())
    {
    ...
    Del(nod -> sons[s[p] - 'a'], p + 1);
}

int Cnt(node* nod = t, int p = 0)
{
    if(p == s.size())return nod -> end;
    if(!nod -> sons[s[p] - 'a'])return 0;
    return Cnt(nod -> sons[s[p] - 'a'], p + 1);
}

int Pre(node* nod = t, int p = 0)
{
    if(p == s.size())return p * (nod -> cnt > 0);
    if(!nod -> sons[s[p] - 'a'])return p * (nod -> cnt > 0);
    return max(p * (nod -> cnt > 0), Pre(nod -> sons[s[p] - 'a'], p + 1));
}
...
void solve_testcase()
{
    ...
    while(cin >> op)
    {
        ...
        else if(op == 2)cout << Cnt() << '\n';
        else if(op == 3)cout << Pre() << '\n';
    }
}
