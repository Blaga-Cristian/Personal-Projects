...
void solve_testcase()
{
    ...
    FOR(i, 1, n)nor.pb(a[i]);
    sort(all(nor));
    nor.erase(unique(nor.begin(), nor.end()), nor.end());

    FOR(i, 1, n)a[i] = distance(nor.begin(), lower_bound(nor.begin(), nor.end(), a[i])) + 1;
    ...
    cout << dp[bst] << '\n';
    vi ans;
    while(bst)ans.pb(a[bst]), bst = t[bst];
    reverse(ans.begin(), ans.end());
    for(auto i : ans)cout << nor[i - 1] << ' ';
}
