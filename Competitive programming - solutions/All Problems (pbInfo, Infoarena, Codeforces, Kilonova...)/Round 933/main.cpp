...
void solve_testcase()
{
    ...
    FOR(st, 1, m)
    {
        while(dr > 0 && d[st] + f[dr] > a[ind])-- dr;
        while(dr > 1 && d[st] + f[dr - 1] >= a[ind - 1] &&
              d[st] + f[dr - 1] - a[ind - 1] >= a[ind] - d[st] - f[dr - 1])-- dr;

        if(dr > 0 && d[st] + f[dr] <= a[ind] && d[st] + f[dr] >= a[ind - 1])cand = min(cand, max(d[st] + f[dr] - a[ind - 1],
                                                                                                 a[ind] - d[st] - f[dr]));
        if(dr > 1 && d[st] + f[dr - 1] <= a[ind] && d[st] + f[dr - 1] >= a[ind - 1])cand = min(cand, max(d[st] + f[dr - 1] - a[ind - 1],
                                                                                                     a[ind] - d[st] - f[dr - 1]));
    }

    FOR(i, 2, n)
        if(i != ind)
            cand = max(cand, a[i] - a[i - 1]);

    cout << max(1LL, min(mx, cand)) << '\n';
}
