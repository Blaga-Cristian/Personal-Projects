...
bool testCase = false;
const ll N = 509, M = 5009, LG = 31, Mod = 998244353;

int n, m, arr[N][N], tot[N], tmp[N], dp[N][N][N];
///arr[i][j] = nr of ways to dist 1...i st 2 has j cards more than i
///dp[i][j] = nr of ways to dist 1...i st 1 has i ans 2 has j, with the useless prefix
...
void solve_testcase()
{
    ...
    FOR(i, 1, m)
        FOR(j, 0, m)
        {
            if(j + 1 <= m)arr[i][j] = (arr[i][j] + arr[i - 1][j + 1]) % Mod;
            if(j - 1 >= 0)arr[i][j] = (arr[i][j] + arr[i - 1][j - 1]) % Mod;
        }
    ...
    FOR(i, 3, n)
    {
        ...
        FOR(j, 0, m)
            FOR(k, 0, m)
                if(j + k <= m)
                    tmp[j + k] = (tmp[j + k] + 1LL * tot[j] * arr[m][k] % Mod) % Mod;

        FOR(j, 0, m)tot[j] = tmp[j];
    }
    ...
    FOR(i, 1, m)
        FOR(j, 0, m)
            FOR(k, 0, m)
                if(j + k <= i)
                {
                    ///put it in the prefix
                    if(j == 0)dp[i][j][k] = (dp[i][j][k] + dp[i - 1][j][k]) % Mod;

                    ///give it to 1
                    if(j <= k && j)dp[i][j][k] = (dp[i][j][k] + dp[i - 1][j - 1][k]) % Mod;
                    ///give it to 2
    ...
    FOR(j, 0, m)
        if((m - j) % 2 == 0)
            ans = (ans + 1LL * tot[j] * dp[m][(m - j) / 2][(m - j) / 2]) % Mod;
