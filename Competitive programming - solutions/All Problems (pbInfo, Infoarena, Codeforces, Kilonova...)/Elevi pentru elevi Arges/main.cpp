...
int dp[N][2][2];
///dp[x][ok1][ok2] =
///suma nr de lungime x mai mici decat ce am construit pana acum a.i.
///ok1 am ales deja un nr mai mare ca a
///ok2 am ales deja un nr mai mic ca b
string a, b;
...
void solve_testcase()
{
    ...
    for(int i = 1; i < n; ++i)
    {
        if(a[i] == b[i] && a[i] == '1')///1 1
        {
            dp[i][0][0] = dp[i - 1][0][0];
            dp[i][1][0] = dp[i - 1][1][0] + 1;
            dp[i][0][1] = dp[i - 1][0][1];
            dp[i][1][1] = dp[i - 1][1][1] * 2 + ///pun 0
                            dp[i - 1][1][1] + 1; ///pun 1
//                            dp[i - 1][0][1] * 0 +
//                            dp[i - 1][1][0] * 2; ///pun 0
        }
        else if(a[i] == b[i] && a[i] == '0')///0 0
        {
            dp[i][0][0] = dp[i - 1][0][0];
...
//            dp[i][0][0] = 0;
            dp[i][1][0] = dp[i - 1][1][0] + 1 +
                            dp[i - 1][0][0] + 1;
//            dp[i][0][1] = 0; - n-ar trebui sa conteze
            dp[i][1][1] = dp[i - 1][1][1] * 2 +
                            dp[i - 1][1][1] + 1 +
                            dp[i - 1][1][0] * 2 +
                            dp[i - 1][0][1] + 1;
...
//                            dp[i - 1][0][1]
    ...
    cout << dp[n - 1][1][0] << '\n'; // - e problema
//    cout << (dp[n - 1][0][0] + dp[n - 1][0][1] + dp[n - 1][1][0] + dp[n - 1][1][1]) << '\n';
}
