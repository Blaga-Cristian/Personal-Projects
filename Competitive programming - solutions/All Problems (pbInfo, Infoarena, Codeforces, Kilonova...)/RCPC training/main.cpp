...
bool testCase = true;
const ll N = 2e5 + 9, M = 5009, LG = 31, Mod = 1e9 + 7;
...
void Precalc()
{
    ...
    FORR(i, N - 2, 0)inv[i] = (i + 1) * inv[i + 1] % Mod;
}
ll comb(ll n, ll k)
{
...
void solve_testcase()
{
    ...
    {
        for(int j = 0; j <= v[i].ss; ++j)
            cnt[cur][j] = 0;

        for(int j = 1; j <= v[i].ss; ++j)
            cnt[prv][j] = cnt[prv][j - 1] + cnt[prv][j];

        for(int j = 0; j <= v[i].ss; ++j)
        {
             += comb(fr[v[i].ff], j) * j * cnt[prv][j];
            cnt[cur][j] = comb(fr[v[i].ff], j) * cnt[prv][j];
        }
