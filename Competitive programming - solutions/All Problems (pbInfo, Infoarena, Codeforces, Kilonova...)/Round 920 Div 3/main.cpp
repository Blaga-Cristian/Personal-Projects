...
ll n, q, a[N], cresc[N][bucket], sum[N][bucket];
...
void solve_testcase()
{
    ...
    queries = vector<info>(q);
    FOR(i, 0, q - 1)
    {
        cin >> queries[i].s >> queries[i].step >> queries[i].nr;
        queries[i].ans = 0;
    }

    for(auto& q : queries)
        if(q.step >= bucket)
            for(int i = q.s, nr = 1; nr <= q.nr; nr ++, i += q.step)
                q.ans += nr * a[i];
    ...
    FORR(i, n, 1)
        ...
        {
            cresc[i][s] = a[i];
            if(i + s <= n)cresc[i][s] += cresc[i + s][s] + sum[i + s][s];
            sum[i][s] = a[i];
            if(i + s <= n)sum[i][s] += sum[i + s][s];
        }

    for(auto& q : queries)
        if(q.step < bucket)
        {
            q.ans = cresc[q.s][q.step];
            if(q.s + q.step * q.nr <= n)q.ans -= cresc[q.s + q.step * q.nr][q.step] + q.nr * sum[q.s + q.step * q.nr][q.step];
        }
