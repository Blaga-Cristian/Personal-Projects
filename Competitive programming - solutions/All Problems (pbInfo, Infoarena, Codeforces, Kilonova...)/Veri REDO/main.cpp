...
void solve_testcase()
{
    ...
    int mn = Inf, nod = -1, ciclu = -1;
    FOR(i, 1, n)
    {
        int cand = Inf, c_nod = i, c_ciclu = -1;

        FOR(j, 1, n)
            if(j != i && cand > d[s][i] + d[i][j] + d[j][i] + max(d[i][a], d[i][b]))
            {
                cand = d[s][i] + d[i][j] + d[j][i] + max(d[i][a], d[i][b]);
                c_ciclu = j;
            }
    ...
    {
        vi v;
        get(s, nod, v), v.pop_back(), get(nod, ciclu, v), v.pop_back(), get(ciclu, nod, v);
