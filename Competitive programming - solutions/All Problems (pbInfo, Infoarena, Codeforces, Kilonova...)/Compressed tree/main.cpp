...
bool testCase = true;
const ll Nmax=5e5+9, Inf = 1e18;

ll n, a[Nmax];
ll ans;
ll dp[Nmax];
/// dp[0][x] - suma maxima din subarborele x daca are gradul mai mare ca 1
/// dp[1][x] - suma maxima din subarborele x daca are gradul egal cu 1
/// dp[2][x] - suma maxima din subarborele x daca are gradul egal cu 0
vvi G(Nmax);
...
void Dfs(int x = 1, int p = 0)
{
    ...
    for(auto y : G[x])
        if(y != p)
        {
            Dfs(y, x);
            for(int i = 3; i >= 0; --i)
                sum[min(i + 1, 3)] = max(sum[min(i + 1, 3)], sum[i] + dp[y]);
        }

    dp[x] = -Inf;
    for(int j = 0; j < 4; ++j)
    {
        dp[x] = max(dp[x], sum[j] + ((j == 1) ? 0 : a[x]));
        ans = max(ans, sum[j] + ((j == 2) ? 0 : a[x]));
    }
}
