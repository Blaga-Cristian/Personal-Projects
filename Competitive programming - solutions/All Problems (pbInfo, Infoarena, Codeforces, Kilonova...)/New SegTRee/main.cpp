...
void Build(int n, ll* arr, int index = 0)
{
    h = sizeof(int) * 8 - __builtin_clz(n);
    for(int p = 0; p < 2 * n; ++p)d[p] = -1;
    for(int p = 0; p < n; ++p)t[n + p] = arr[p + index];
    for(int p = n - 1; p; --p)t[p] = min(t[p << 1], t[p << 1 | 1]);
}

void Build(int p)
{
    for(; p > 1; p >>= 1)if(d[p >> 1] == -1)t[p >> 1] = min(t[p], t[p ^ 1]);
}
...
void Update(int l, int r, ll val, int index = 0)
{
    l += n - index, r += n - index;
    int l0 = l, r0 = r;
    Push(l);
...
ll Query(int l, int r, int index = 0)
{
    l += n - index, r += n - index;
    Push(l);
    Push(r);
