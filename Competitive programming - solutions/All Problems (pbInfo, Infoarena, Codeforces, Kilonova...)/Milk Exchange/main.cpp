...
void solve_testcase()
{
    ...
    deque<pair<ll, pair<ll, ll>>> dq;///indicele + range-ul l, r
    FOR(i, 1, n)
    {
        while(!dq.empty() && a[dq.back().ff] > a[i])dq.pop_back();
        if(dq.empty())dq.push_back({i, {1, i}});
        else dq.push_back({i, {dq.back().ss.ss + 1, i}});
    }

    FOR(i, 1, n)
    {
        if(dq.front().ff == i)dq.pop_front();

//        cout << "At point " << i << " we have ";
//        for(auto j : dq)cout << j.ff << ' ';
//        cout << '\n';

        pii range = {i, i};
        while(!dq.empty() && a[dq.back().ff] > a[i])
        {
            mars[dist(dq.back().ff, i)] -= a[dq.back().ff] - a[i];
            mars[min(n + 1, dist(dq.back().ss.ff, i) + 1)] += a[dq.back().ff] - a[i];
            range.ff = dq.back().ss.ff;

//            cout << "Update from " << dist(dq.back().ss.ff, i) << " to " << dist(dq.back().ss.ff, i) << ' ' << a[dq.back().ff] - a[i] << '\n';
            dq.pop_back();
        }
        dq.push_back({i, range});
