...
bool mat(int x, int y, int x1, int y1)
{
    return (x == x1 && y == y1);
}
...
void Back(int x = 0, int y = 0)
{
    ...
    if(sp[x])
    {
        if(!usd[a[x][y]][a[x][y + 1]])
        {
            usd[a[x][y]][a[x][y + 1]] = true;
            if(y + 2 >= 8)
                Back(x + 1, 0);
            else Back(x, y + 2);
        }
    ...
    else
    {
        FOR(i, 0, 6)
            FOR(j, 0, 6)
                if(!usd[i][j] && mat(i, j, a[x][y], a[x][y + 1]))
                {
                    usd[i][j] = true;
                    FOR(k, 0, 6)
                        FOR(l, 0, 6)
                            if(!usd[k][l] && mat(k, l, a[x + 1][y], a[x + 1][y + 1]))
                            {
                                usd[k][l] = true;
                                if(y + 2 >= 8)Back(i + 2, 0);
                                else Back(i, y + 2);
        ...
        FOR(i, 0, 6)
            FOR(j, 0, 6)
                if(!usd[i][j] && mat(i, j, a[x][y], a[x + 1][y]))
                {
                    usd[i][j] = true;
                    FOR(k, 0, 6)
                        FOR(l, 0, 6)
                            if(!usd[k][l] && mat(k, l, a[x][y + 1], a[x + 1][y + 1]))
                            {
                                usd[k][l] = true;
                                if(y + 2 >= 8)Back(i + 2, 0);
                                else Back(i, y + 2);
