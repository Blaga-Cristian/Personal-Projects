...
void Dijkstra()
{
    ...
    while(!q.empty())
    {
        int dx = q.top().ff, x = q.top().ss.ff, src = q.top().ss.ss;
        q.pop();

        if(!d[x].count(src) || dx > d[x][src])continue;

        for(auto [y, dst] : G[x])
        {
            int new_d = d[x][src] + dst;
            if(new_d > r)continue;
            if(!d[y].count(src) || d[y][src] > new_d)
            {
                d[y][src] = new_d;
                if(d[y].size() > k)
                {
                ...
                if(d[y].count(src))q.push({d[y][src], {y, src}});
            }
        }
    }
}
...
void solve_testcase()
{
    ...
    vi ans;
    FOR(i, c + 1, n)if(d[i].size() == k)ans.pb(i);
    cout << ans.size() << '\n';
