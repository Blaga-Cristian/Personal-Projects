...
bool testCase = false;
const int N = 3e5 + 9, LG = 31, BLOCK = 500, Mod = 1e9 + 9;

int n, m, a[N], sp[N], d[N];
int F[N], S[N];
...
void solve_testcase()
{
    ...
    int op, l, r;
    vpi v;
    for(int k = 1; k <= m; k += BLOCK)
    {
        for(int i = k; i <= min(m, k + BLOCK - 1); ++i)
        {
            cin >> op >> l >> r;
            ...
            {
                int ans = sp[r] - sp[l - 1] + Mod;
                if(ans >= Mod)ans -= Mod;

                for(auto [st, dr] : v)
                    if(l <= st && st <= r)
                    {
                        ans = ans + S[min(dr - st + 1, r - st + 1)];
                        if(ans >= Mod)ans -= Mod;
                    }
                    else if(st < l && l <= dr)
                    {
        ...
        {
            d[st] += 1;
            if(d[st] >= Mod)d[st] -= Mod;
            d[dr + 1] += Mod - F[dr - st + 2];
            if(d[dr + 1] >= Mod)d[dr + 1] -= Mod;
            d[dr + 2] += Mod - F[dr - st + 1];
            if(d[dr + 2] >= Mod)d[dr + 2] -= Mod;
        }
