...
void solve_poz()
{
    FOR(i, 1, n - 1)moves.eb(i + 1, i);
}
void solve_neg()
{
    FORR(i, n, 2)moves.eb(i - 1, i);
}
void afis()
{
    cout << moves.size() << '\n';
    for(auto i : moves)cout << i.ff << ' ' << i.ss << '\n';
}
...
void solve_testcase()
{
    ...
    int m_s = 0, m_b = 0, cnt_neg = 0, cnt_poz = 0, aux;

    FOR(i, 1, n)
        if(p[i] < 0)cnt_neg ++;
        else if(p[i] > 0)cnt_poz ++;
    ...
    if(m_s + cnt_poz < m_b + cnt_neg)
    {
        FOR(i, 1, m_s)moves.eb(pmn, pmn);
        FOR(i, 1, n)if(p[i] > 0)moves.eb(i, pmn);
        solve_neg();
    }
    else
    {
        FOR(i, 1, m_b)moves.eb(pmx, pmx);
        FOR(i, 1, n)if(p[i] < 0)moves.eb(i, pmx);
        solve_poz();
    }
