...
{
    return (a.st / block == b.st / block) ? a.dr < b.dr : a.st < b.st;
}
...
bool ok(int x, int i)
{
    return ((x >= 0) && (x < vecs[i].size()));
}
void AddD(int i)
{
//    cout << __PRETTY_FUNCTION__ << '\n';
    i = id[s[i]];
    //if(ok(pl[i], i) && ok(pr[i], i))mys.erase(mys.find(vecs[i][pr[i]] - vecs[i][pl[i]]));
    pr[i]++;
    //if(ok(pl[i], i) && ok(pr[i], i))mys.insert(vecs[i][pr[i]] - vecs[i][pl[i]]);
}
void RemD(int i)
{
//    cout << __PRETTY_FUNCTION__ << '\n';
...
void solve_testcase()
{
    ...
    s[0] = 0;
    if(!id[s[0]])id[s[0]] = ++NEXT_FREE_INDEX;
    vecs[id[s[0]]].pb(0);
    FOR(i, 1, n)
    {
        s[i] = s[i - 1] ^ a[i];
        if(!id[s[i]])id[s[i]] = ++NEXT_FREE_INDEX;
        vecs[id[s[i]]].pb(i);
    }
...
//    }
    ...
    for(auto& e : qs)
    {
        while(dr < e.dr)AddD(++dr);
        while(st > e.dr)AddS(--st);
        while(dr > e.dr)RemD(dr--);
        while(st < e.st)RemS(st++);

//        cout << e.st << ' ' << e.dr << '\n';
//        FOR(i, 1, 3)cout << pl[i] << ' ' << pr[i] << '\n';
//        cout << '\n';

        if(!mys.empty())e.ans = *mys.begin();
    }
