...
void Precalc()
{
    ...
    while(!q.empty())
    {
        ...
        FOR(k, 0, 3)
        {
            ...
            if(inmat(nx, ny) && d[nx][ny] > d[x][y] + 1)
            {
                d[nx][ny] = d[x][y] + 1;
                q.emplace(nx, ny);
            }
...
bool ok(int dmin, int startx, starty)
{
    queue<pii> q;

    FOR(i, 1, n)FOR(j, 1, m)viz[i][j] = 0;
    ...
    while(!q.empty())
    {
        ...
        {
            int nx = x + di[k], ny = y + dj[k];
            if(inmat(nx, ny) && !island[nx, ny] && !viz[nx][ny])
            {
                viz[nx][ny] = 1;
                q.emplace(nx, ny);
            }
