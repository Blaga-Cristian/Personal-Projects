...
void Precalc()
{
    FOR(i, 1, n)layer[0].pb(i);

    FOR(l, 1, LG)
    {
        if(l % 2) /// minima remain
        {
            FOR(i, 0, layer[l - 1].size())
            {
                bool is_minima = true;
                if(i > 0 && layer[l - 1][i - 1] < layer[l - 1][i])is_minima = false;
                if(i + 1 < layer[l - 1].size() && layer[l - 1][i + 1] < layer[l - 1][i])is_minima = false;

                if(is_minima)layer[l]pb(i);
            }
        }
        else /// maxima remain
        {
