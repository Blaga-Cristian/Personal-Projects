...
ll n, a[Nmax], dp[2][Nmax];
vvi G(Nmax);

/// dp[0][x] - suma maxima din subarborele x
/// dp[1][x] - suma maxima din subarborele x daca gradul lui x este 1
...
void Dfs(int x = 1, int p = 0)
{
    ...
    for(auto y : G[x])
    {
        ...
        if(G[y].size() > 1)
            mst.insert(max(a[y], max(dp[0][y], dp[1][y] - a[y])));
        else mst.insert(a[y]);

        if(G[y].size() > 1)
            dp[1][x] = max(dp[1][x], max(a[x] + a[y], max(dp[1][y] - a[y] + a[x], a[x] + dp[0][y])));
        else dp[1][x] = max(dp[1][x], a[x] + a[y]);
    }
...
//    FOR(i, 1, n)
//    {
//        cout << "For node " << i << ' ' << dp[0][i] << ' ' << dp[1][i] << '\n';
//    }

    ll ans = 0;
    FOR(i, 1, n)ans = max(ans, max(dp[1][i], dp[0][i]));
    cout << ans << '\n';
}
