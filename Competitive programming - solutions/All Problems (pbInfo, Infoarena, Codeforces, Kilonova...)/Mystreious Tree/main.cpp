...
void add(node* nod, int x, int lvl = LG)
{
    nod -> cnt_f ++;
    ...
    if(!nod -> sons[bit])nod -> sons[bit] = new node();

    add(nod -> sons[bit], x, lvl - 1);
}

void rem(node* nod, int x, int lvl = LG)
{
    nod -> cnt_f --;
    ...
    rem(nod -> sons[bit], x, lvl - 1);
}

int fmax(node* nod, int x, int lvl = LG)
{
    if(lvl == -1)return 0;

    int bit = ((x & (1 << lvl)) != 0);

    if(nod -> sons[!bit] && nod -> sons[!bit] -> cnt_f)return (1 << lvl) + fmax(nod -> sons[!bit], x, lvl - 1);
    else if(nod -> sons[bit] -> cnt_f)return fmax(nod -> sons[bit], x, lvl - 1);
    return 0;
}
...
void solve_testcase()
{
    ...
    FOR(i, 1, m)
    {
        ...
        else
        {
            ...
            ///iau de aceeasi paritate
            int ans1 = fmax(t[dep[v] % 2], x ^ sum[v]);
            ///iau de pariate dif
            int ans2 = fmax(t[1 - dep[v] % 2], x ^ sum[v] ^ toxor);
