...
void solve_testcase()
{
    ...
    ll scor = 0;
    FOR(i, 1, n)scor += a[i] - b[i];

    FOR(i, 1, n)sa.insert({a[i], i});
    FOR(i, 1, n)sb.insert({b[i], i});
    ...
    FOR(k, 1, n)
    {
        ...
        sa.erase(sa.find({a[p.ss], p.ss}));
        sb.erase(sb.find({b[p.ss], p.ss}));
        turn = 1 - turn;
    }
