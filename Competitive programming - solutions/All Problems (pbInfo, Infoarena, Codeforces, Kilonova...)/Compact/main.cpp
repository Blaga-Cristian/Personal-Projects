...
int n, m, a[N], fst[N], lst[N], t[N];

struct node
{
    int mn, mx;

    node(int mn = Inf, int mx = -Inf) : mn(mn), mx(mx) {}

    node operator + (node const& oth)
    {
        return {min(mn, oth.mn), max(mx, oth.mx)};
    }
};
node ind[4 * N], val[4 * N];

void Update(node* tree, int p, int mn, int mx, int nod = 1, int st = 1, int dr = n)
{
    if(st == dr)
    {
        tree[nod].mn = mn;
    ...
    int m = (st + dr) >> 1;
    if(p <= m)Update(tree, p, mn, mx, nod << 1, st, m);
    else Update(tree, p, mn, mx, nod << 1 | 1, m + 1, dr);
    tree[nod] = tree[nod << 1] + tree[nod << 1 | 1];
}

node Query(node* tree, int l, int r, int nod = 1, int st = 1, int dr = n)
{
    if(l <= st && dr <= r)return tree[nod];
    int m = (st + dr) >> 1;
    if(r <= m)return Query(tree, l, r, nod << 1, st, m);
    else if(l > m)return Query(tree, l, r, nod << 1 | 1, m + 1, dr);
    return Query(tree, l, r, nod << 1, st, m) + Query(tree, l, r, nod << 1 | 1, m + 1, dr);
}
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)
    {
        Update(ind, i, a[i], a[i]);
        Update(val, a[i], fst[a[i]], lst[a[i]]);
    }

    for(int i = 1; i <= n; ++i)
    {
        int l = i, r = i, lo = a[i], hi = a[i];
        while(true)
        {
            int nl = Query(val, lo, hi).mn;
            int nr = Query(val, lo, hi).mx;
            if(l <= nl && nr <= r)break;
            l = nl, r = nr;
            lo = Query(ind, l, r).mn;
            hi = Query(ind, l, r).mx;
