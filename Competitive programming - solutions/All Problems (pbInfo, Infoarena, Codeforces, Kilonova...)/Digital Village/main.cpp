...
const ll Inf = 1e18;
bool testCase = true;
const ll N = 2e5 + 9, LG = 31, Mod = 1e9 + 7;
...
void solve_testcase()
{
    ...
    bst = sp = a = dp[0] = dp[1] = vvl(n + 2, vl(m + 2));

    FOR(i, 1, n)FOR(j, 1, m)cin >> a[i][j];
    FOR(i, 1, n)FOR(j, 1, m)sp[i][j] = sp[i][j - 1] + a[i][j];

    FOR(j, 1, m)dp[1][1][j] = max(a[1][j], a[1][j] + dp[1][1][j - 1]);
    FORR(j, m, 1)dp[0][1][j] = max(a[1][j], a[1][j] + dp[0][1][j + 1]);
    FOR(b, 0, 2)dp[b][1][0] = dp[b][1][m + 1] = -Inf;

//    ///dp[0][i][j] - cel mai bun care incepe la j, / 1 - cel mai bun care se termina la j
    FOR(i, 2, n)
    {
        int cand = -Inf;
        FOR(j, 1, m)bst[i][j] = max(a[i][j], a[i][j] + bst[i][j - 1]);

        FOR(j, 1, m)
        {
            if(j > 1)cand = max(cand, dp[0][i - 1][j] + bst[i][j - 1] - sp[i][j - 1]);
            if(j > 1)cand = max(cand, dp[1][i - 1][j - 1] + bst[i][j - 1] - sp[i][j - 1]);
            dp[1][i][j] = cand + sp[i][j];
        }

        FORR(j, m, 1)bst[i][j] = max(a[i][j], a[i][j] + bst[i][j + 1]);

        cand = -Inf;
        FORR(j, m, 1)
        {
            if(j < m)cand = max(cand, dp[0][i - 1][j + 1] + bst[i][j + 1] - (sp[i][m] - sp[i][j]));
            if(j < m)cand = max(cand, dp[1][i - 1][j] + bst[i][j + 1] - (sp[i][m] - sp[i][j]));
            dp[0][i][j] = cand + sp[i][m] - sp[i][j - 1];
        }
    }

    int ans = -Inf;
    FOR(j, 1, m)ans = max(ans, dp[0][n][j]);
    cout << ans << '\n';
}
