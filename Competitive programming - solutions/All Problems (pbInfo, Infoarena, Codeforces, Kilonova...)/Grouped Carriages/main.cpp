...
int n, m, k, h[Nmax], in[Nmax], out[Nmax];
pii dp[Nmax];
pii mn[Nmax];
vvi G(Nmax);
vector<vector<pii>> op(Nmax);
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)
    {
        ...
        sort(op[x].begin(), op[x].end());
        for(auto e : op[x])
        {
            if(e.ff > dp[x].ff)dp[x] = e;
            else
            {
                if(e.ss < dp[x].ss)dp[x].ff ++;
                dp[x].ss = e.ss;
            }
        }

        if(dp[x].ss > h[x])dp[x].ff ++;
        dp[x].ss = h[x];
    ...
    bool day_one = true;
    FOR(i, 1, n)if(dp[i].ff != 0)day_one = false;

    if(day_one)
    {
        vi v;
        FOR(i, 1, n)if(!out[i])v.pb(i);

        sort(all(v), [](int a, int b){return mn[a].ss < mn[b].ss;});

        int ans = 0;
        FOR(i, 1, n)ans = max(ans, dp[i].ss);

        int mx_dp = dp[1].ss;
        FOR(i, 2, v.size() - 1)
        {
            ans = min(ans, k - mn[i].ss + 1 + mx_dp);
            mx_dp = max(mx_dp, dp[i].ss);
        }
    ...
    pii ans = {0, 0}, mn = {INT_MAX, INT_MAX};
    FOR(i, 1, n)
    {
        ans = max(ans, dp[i]);
        mn = min(mn, dp[i]);
    ...
    cout << 1LL * ans.ff * k + ans.ss - mn.ss << '\n';
}
