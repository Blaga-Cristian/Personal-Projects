...
void Build(int nod = 1, int st = 1, int dr = n)
{
    ...
    tree[nod].v = tree[nod << 1].v + tree[nod << 1 | 1].v;
}

void Push(int nod, int st, int dr)
{
    if(st == dr || (tree[nod].ladd == 0 && tree[nod].lass == 0))return;

    int m = (st + dr) >> 1;
    ll vadd = tree[nod].ladd, vass = tree[nod].lass;

    if(vass == 0)
    {
        tree[nod << 1].v += (m - st + 1) * vadd;
        tree[nod << 1].ladd += vadd;
        tree[nod << 1 | 1].v += (dr - m) * vadd;
        tree[nod << 1 | 1].ladd += vadd;
    }
    else
    {
        tree[nod << 1].v = (m - st + 1) * (vass + vadd);
        tree[nod << 1].lass = vass;
        tree[nod << 1].ladd = vadd;
        tree[nod << 1 | 1].v = (dr - m) * (vass + vadd);
        tree[nod << 1 | 1].lass = vass;
        tree[nod << 1 | 1].ladd = vadd;
    }

    tree[nod].ladd = tree[nod].lass = 0;
}

void Update(int l, int r, ll val, bool tip, int nod = 1, int st = 1, int dr = n)///tip == 0 -> add, tip == 1 -> ass
{
    if(r < st || l > dr)return;
    if(l <= st && dr <= r)
    {
    ...
    int m = (st + dr) >> 1;
    Update(l, r, val, tip, nod << 1, st, m);
    Update(l, r, val, tip, nod << 1 | 1, m + 1, dr);
...
void solve_testcase()
{
    ...
    {
        cin >> op >> a >> b;
        if(op == 1)cin >> x, Update(a, b, x, 0);
        else if(op == 2)cin >> x, Update(a, b, x, 1);
        else cout << Query(a, b) << '\n';
    }
