...
bool testCase = true;
const ll N = 1e5 + 9, K = 20, LG = 19;

int n, k, dp[(1 << K) + 1], le[K], re[K], lca[K];///le = left edge for the lca pair, re = ...
pii p[K + 9];
vvi G(N);

int t[N], ST[2 * N][LG], tour[2 * N], sz, first[N], dep[N], in[N], out[N], id2;
...
void Precalc()
{
    ...
    FOR(i, 1, 2 * n - 1)ST[i][0] = tour[i];

    for(int p = 1; (1 << p) <= 2 * n - 1; ++p)
        for(int i = 1; i + (1 << p) - 1 <= 2 * n - 1; ++i)
            if(dep[ST[i][p - 1]] < dep[ST[i + (1 << (p - 1))][p - 1]])ST[i][p] = ST[i][p - 1];
...
int get_lca(int x, int y)
{
    ...
    if(x > y)swap(x, y);
    int p = log2(y - x + 1);
    return dep[ST[x][p]] < dep[ST[y - (1 << p) + 1][p]] ? ST[x][p] : ST[y - (1 << p) + 1][p];
}
bool is_ancestor(int t, int x)
{
    return in[t] <= in[x] && out[x] <= out[t];
}
...
void solve_testcase()
{
    ...
    cin >> k;
    FOR(i, 1, k)cin >> p[i].ff >> p[i].ss;

    Precalc();

    FOR(i, 1, k)
    {
        lca[i] = get_lca(p[i].ff, p[i].ss);
        for(auto y : G[lca[i]])
        {
            if(y == t[lca[i]])continue;

            if(p[i].ff == lca[i] || is_ancestor(y, p[i].ff))le[i] = y;
            if(p[i].ss == lca[i] || is_ancestor(y, p[i].ss))re[i] = y;
        }
    }

    vector<info> v;
    FOR(i, 1, k)v.pb({p[i].ff, p[i].ss, lca[i]});
    sort(all(v), [](info a, info b){return dep[a.lca] > dep[b.lca];});

    FOR(i, 0, (1 << k) - 1)dp[i] = Inf;
    dp[0] = dp[1] = 1;
    ...
    for(int i = 1; i < k; ++i)
        for(int s = 0; s < (1 << i); ++s)
        {
            bool covl = false;
            for(int k = 0; k < i; ++k)
                if((!(s & (1 << k)) && is_ancestor(le[k + 1], lca[]))
        }
