...
bool testCase = true;
const ll N = 2e5 + 9, K = 22, LG = 31, Mod = 1e9 + 7;
...
void solve_testcase()
{
    ...
    FOR(i, 0, n)FOR(j, 0, k)FOR(b, 0, 2)dp[i][j][b] = LLONG_MIN >> 4;
    ...
    {
        dp[i][0][0] = dp[i - 1][0][0];
        dp[i][0][1] = max(dp[i - 1][0][1] + a[i] - x, a[i] - x);
        dp[i][0][2] = max({dp[i - 1][0][0],
                          dp[i - 1][0][1],
                          dp[i - 1][0][2]});
