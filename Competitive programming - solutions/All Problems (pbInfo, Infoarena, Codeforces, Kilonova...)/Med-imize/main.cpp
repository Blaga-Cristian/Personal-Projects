...
bool testCase = true;
const ll N = 1e6 + 9, LG = 31, Mod = 1e9 + 7;
...
int calc(int val)
{
    ...
    {
        l[i] = l[i - 1] + (a[i] < val);
        if(i >= k)l[i] = min(l[i], l[i - k]);
    }

    FORR(i, n, 1)
    {
        r[i] = r[i + 1] + (a[i] < val);
        if(i <= n - k + 1)r[i] = min(r[i], r[i + k]);
    }

    int ret = Inf;
    FOR(i, 1, n)
        ret = min(ret, (a[i] < val) + l[i - 1] + r[i + 1]);
    return ret;
}
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)a[i] = distance(norm.begin(), lower_bound(norm.begin(), norm.end(), a[i])) + 1;
