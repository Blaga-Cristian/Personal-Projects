...
void add(string& s, int p = 0, int i = 0)///len = (strlen(s) + 1)
{
    t[i].dp += s.size() - p;
    t[i].cnt ++;
    ...
    if(!sons[i][s[p] - 'a'])sons[i][s[p] - 'a'] = ++NEXT_FREE_IDX;

    add(s, p + 1, sons[i][s[p] - 'a']);
}

ll get(string& s, int p = 0, int i = 0)
{
    if(p == s.size())return t[i].dp;

    if(!sons[i][s[p] - 'a'])return t[i].dp + t[i].cnt * (s.size() - p);

    return t[i].dp - (t[sons[i][s[p] - 'a']].dp + t[sons[i][s[p] - 'a']].cnt) +
            (t[i].cnt - t[sons[i][s[p] - 'a']].cnt) * (s.size() - p)  +
            get(s, p + 1, sons[i][s[p] - 'a']);
}

void clear()
{
    FOR(i, 0, NEXT_FREE_IDX)
    {
        t[i].dp = t[i].cnt = 0;
        FOR(j, 0, 25)sons[i][j] = 0;
    }
    NEXT_FREE_IDX = 0;
}
