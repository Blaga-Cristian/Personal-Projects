...
node* Update(int p, node* nod, int st = 1, int dr = n)
{
    node *nnod = new node();
    nnod -> l = nod -> l;
    nnod -> r = nod -> r;
    nnod -> sum = nod -> sum + 1;
    if(st == dr)return nnod;
    int m = (st + dr) >> 1;
    if(p <= m)nnod -> l = Update(p, nod -> l, st, m);
    else nnod -> r = Update(p, nod -> r, m + 1, dr);
    return nnod;
}

int Query(node* pos, node* neg, int k, int st = 1, int dr = n)
{
    if(st == dr)return st;
    int m = (st + dr) >> 1;
    if(pos -> l -> sum - neg -> l -> sum >= k)
        return Query(pos -> l, neg -> l, k, st, m);
    else
        return Query(pos -> r, neg -> r, k - (pos -> l -> sum - neg -> l -> sum), m + 1, dr);
}
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)a[i] = distance(v.begin(), lower_bound(all(v), a[i])) + 1;

    r[0] = Build();
    FOR(i, 1, n)r[i] = Update(a[i], r[i - 1]);
