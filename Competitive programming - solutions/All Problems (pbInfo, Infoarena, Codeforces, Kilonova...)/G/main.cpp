...
bool testCase = true;
const ll N = 2e5 + 9, P = 317, M1 = 1e9 + 123, M2 = 1e9 + 321;
...
void solve_testcase()
{
    ...
    FOR(i, 0, n + 1)
    {
        hsh1[i] = hsh2[i] = pt1[i] = pt2[i] = 0;
    }

    FOR(i, 1, n)
    {
        hsh1[i] = add(mul(hsh1[i - 1], P, M1), a[i] - 'a', M1);
        hsh2[i] = add(mul(hsh2[i - 1], P, M2), a[i] - 'a', M2);
    }

    pt1[0] = 1;
    FOR(i, 1, n)pt1[i] = mul(pt1[i - 1], P, M1);

    pt2[0] = 1;
    FOR(i, 1, n)pt2[i] = mul(pt2[i - 1], P, M2);

    int cnt = 0;
    FOR(i, 1, n - 1)
    {
        int add1 = add(hsh1[n], - mul(hsh1[i + 1], pt1[n - i - 1], M1), M1);
        add1 = add(add1, mul(hsh1[i - 1], pt1[n - i - 1], M1), M1);

        int add2 = add(hsh2[n], - mul(hsh2[i + 1], pt2[n - i - 1], M2), M2);
        add2 = add(add2, mul(hsh2[i - 1], pt2[n - i - 1], M2), M2);

        if(st1.find(add1) == st1.end() || st2.find(add2) == st2.end())
        {
            cnt ++;
            st1.insert(add1);
            st2.insert(add2);
