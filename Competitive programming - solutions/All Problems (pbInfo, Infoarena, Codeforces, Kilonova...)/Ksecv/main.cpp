...
bool testCase = false;
const ll N = 1e6 + 9, M = 2100;

ll n, k, m, v[N], a[M], b[M];

ll tmn[N], tmx[N];
void Build(int nod = 1, int st = 0, int dr = n - 1)
{
    if(st == dr)
    {
        tmn[nod] = tmx[nod] = v[st];
        return;
    }
    int m = (st + dr) >> 1;
    Build(nod << 1, st, m);
    ...
    tmn[nod] = min(tmn[nod << 1], tmn[nod << 1 | 1]);
    tmx[nod] = max(tmx[nod << 1], tmx[nod << 1 | 1]);
}

ll mx, mn;
bool check(int nod, int diffmax)
{
    return ((max(mx, tmx[nod]) - min(mn, tmn[nod]) + 1) / 2 <= diffmax);
}
int Query(int l, int diffmax, int nod = 1, int st = 0, int dr = n - 1)
{
    cout << st << ' ' << dr << '\n';
    ...
    if(st == dr)
    {
        if(check(nod, diffmax))return dr;
        return l;
    }
    ...
    if(st >= m)
    {
        ...
        {
            int p = Query(l, diffmax, nod << 1, st, m);
            if(p == m)return max(p, Query(l, diffmax, nod << 1 | 1, m + 1, dr));
            return p;
        }
    }
    else
    {
        if(m < l)return Query(l, diffmax, nod << 1 | 1, m + 1, dr);
        else
        {
            int p = Query(l, diffmax, nod << 1, st, m);
            if(p == m)return max(p, Query(l, diffmax, nod << 1 | 1, m + 1, dr));
...
void solve_testcase()
{
    ...
    FOR(i, 0, n - 1)v[i] = a[i % m] ^ b[i / m];

    Build();


    ll st = 0, dr = 99999999, mij, pos;
    while(st <= dr)
    {
        mij = (st + dr) >> 1;
        if(get_nr(mij) <= k)
