...
tuple<bool, int, int, int, int> sum4(int val,int st,int dr)
{
    for(int i = st; i + 3 <= dr; ++i)
    {
        auto ans = sum3(val - v[i], i + 1, dr);
        bool ok;
        int p1, p2, p3;
        tie(ok, p1, p2, p3) = ans;
        if(ok)
            return MT(true, i, p1, p2, p3);
    }
    return MT(false, 0, 0, 0, 0);
}
...
int main()
{
    ...
    bool ok;
    int p1, p2, p3, p4;
    tie(ok, p1, p2, p3, p4) = ans;

    if(ok)
        cout << p[p1] << ' ' << p[p2] << ' ' << p[p3] << ' ' << p[p4];
    else
        cout << "IMPOSSIBLE";
    return 0;
}
