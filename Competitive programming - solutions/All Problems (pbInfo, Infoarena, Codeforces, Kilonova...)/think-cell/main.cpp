...
void solve_testcase()
{
    ...
    sort(all(v), [](pii a, pii b){return a.ff + a.ss == b.ff + b.ss ? a.ss < b.ss : a.ff + a.ss > b.ff + b.ss;});
    ...
    for(int i = 0; i < v.size();)
    {
        ...
        q.push({v[i].ss, v[i].ff}), ++i;
        while(i < v.size() && v[i].ff + v[i].ss == v[i - 1].ff + v[i - 1].ss)q.push({v[i].ss, v[i].ff}), ++i;

        while(!q.empty())
        {
            int r = q.top().ff, s = q.top().ss;
            q.pop();
            ans.insert(cur);
            -- cur;

            while(i < v.size() && v[i].ff + v[i].ss == cur)q.push({v[i].ss, v[i].ff}), ++i;
        }
    }
