...
void UnBuild(int nod = 1, int st = 1, int dr = n)
{
    Tree[nod].clear();
...
int Query(int l, int r, int val1, int val2, int nod = 1, int st = 1, int dr = n)
{
    if(l <= st && dr <= r)
        return distance(lb(all(Tree[nod]), val1), ub(all(Tree[nod]), val2));

    int mij = (st + dr) >> 1;
    if(r <= mij)return Query(l, r, val1, val2, nod << 1, st, mij);
    else if(l > mij)return Query(l, r, val1, val2, nod << 1 | 1, mij + 1, dr);
    return Query(l, r, val1, val2, nod << 1, st, mij) + Query(l, r, val1, val2, nod << 1 | 1, mij + 1, dr);
}
