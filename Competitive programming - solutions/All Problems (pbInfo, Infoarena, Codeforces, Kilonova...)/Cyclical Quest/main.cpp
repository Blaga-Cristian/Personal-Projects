...
bool testCase = false;
const ll N = 1e5 + 9, M = 1e6 + 9, K = 137, M1 = 1e9 + 7, M2 = 1e9 + 123;
...
int get_hsh(int l, int r){return (hsh[r] - hsh[l] * pt[r - l + 1] % Mod + Mod) % Mod;}
bool cmp(int i, int j)
{
    int st = 0, dr = min(sz - i + 1, sz - j + 1), m, p = -1;
    while(st <= dr)
    {
        int m = (st + dr) / 2;
        if(get_hsh(i, i + m - 1) != get_hsh(j, j + m - 1))
        {
            p = m;
            dr = m - 1;
        }
...
void solve_testcase()
{
    ...
    pt[0] = 1;
    FOR(i, 1, M - 1)pt[i] = 1LL * K * pt[i - 1] % M;
    hsh[0] = 0;

    sz = strlen(s + 1);

    FOR(i, 1, sz)hsh[i] = (1LL * hsh[i - 1] * K % M + s[i] - 'a') % Mod;
    FOR(i, 1, sz)ind[i] = i;
    sort(ind + 1, ind + n + 1, [](int i, int j){return cmp(i, j);}

    FOR(i, 1, n)
    {
        cin >> (x + 1);
        int ln = strlen(x + 1);
        FOR(i, 1, ln - 1)x[ln + i] = x[i];

        int st = 1, dr = sz, m, p = sz + 1;
        while(st <= dr)
        {
            int m = (st + dr) / 2;
            if(!comp(ind[st]))
