...
bool testCase = false;
const ll K = 100, Mod = 1e9 + 7;

ll n, k;


int init[K][K] = {}, mat[K][K] = {}, aux[K][K] = {};
...
{
    fact[0] = 1;
    FOR(i, 1, K - 1)fact[i] = 1LL * i * fact[i - 1] % Mod;

    inv[K - 1] = PtLg(fact[K - 1], Mod - 2);
    FORR(i, K - 2, 0)inv[i] = 1LL * (i + 1) * inv[i + 1] % Mod;
}
ll comb(ll n, ll k)
{
    if(n < k)return 0;
...
void solve_testcase()
{
    ...
    if(n == 2)
    {
        cout << (1 + 1LL * 2 * PtLg(2, k)) % Mod << '\n';
        return;
    }

    Precalc();

    FOR(i, 0, k)init[0][i] = 1LL * PtLg(1, i) % Mod;
    FOR(i, 0, k)init[0][k + 1 + i] = 2LL * PtLg(2, i) % Mod;

    init[0][2 * k + 2] = (init[0][k] + init[0][2 * k + 1]) % Mod;
    ///Matricea init e gata

    FOR(i, 0, k)mat[k + 1 + i][i] = 1;

    FOR(j, 0, k)
    {
        FOR(i, 0, k)mat[i][k + 1 + j] = 1LL * comb(j, i) * PtLg(2, j - i) % Mod;
        FOR(i, 0, k)mat[k + 1 + i][k + 1 + j] = comb(j, i);
    }

    ///Pentru suma
    FOR(i, 0, k)mat[i][2 * k + 2] = 1LL * comb(k, i) * PtLg(2, k - i) % Mod;
    FOR(i, 0, k)mat[k + 1 + i][2 * k + 2] = comb(k, i);
    mat[2 * k + 2][2 * k + 2] = 1;
    ...
    for(; n; n >>= 1)
    {
        if(n & 1)
        {
            FOR(i, 0, 0)
                FOR(j, 0, 2 * k + 2)
                {
                    ...
                    FOR(_, 0, 2 * k + 2)
                    {
                        aux[i][j] = aux[i][j] + 1LL * init[i][_] * mat[_][j] % Mod;
                        if(aux[i][j] >= Mod)aux[i][j] -= Mod;
                    }
                }

            FOR(i, 0, 0)FOR(j, 0, 2 * k + 2)init[i][j] = aux[i][j];
        }
        ...
        FOR(i, 0, 2 * k + 2)
            FOR(j, 0, 2 * k + 2)
            {
                ...
                FOR(_, 0, 2 * k + 2)
                {
                    aux[i][j] = aux[i][j] + 1LL * mat[i][_] * mat[_][j] % Mod;
                    if(aux[i][j] >= Mod)aux[i][j] -= Mod;
                }
            }

        FOR(i, 0, 2 * k + 2)FOR(j, 0, 2 * k + 2)mat[i][j] = aux[i][j];
    }
