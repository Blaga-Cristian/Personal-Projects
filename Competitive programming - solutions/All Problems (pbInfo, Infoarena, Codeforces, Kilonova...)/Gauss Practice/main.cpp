...
bool testCase = true;
const ll N = 109, LG = 60, V = 500;

int n, m, k;
int a[N][N];

ll add(ll a, ll b, ll Mod = k)
{
    ll x = (a + b) % Mod;
    if(x < 0)x += Mod;
    return x;
}
ll mul(ll x, ll y, ll Mod = k)
{
    return (x * y) % Mod;
}
ll PtLg(ll base, ll exp, ll Mod = k)
{
    ll ret = 1;
...
cc_hash_table<int, int> hs;
int inv(int nr)
{
    if(hs.find(nr) != hs.end())return hs[nr];
    return hs[nr] = (gcd(nr).ff + k) % k;
}
...
void solve_testcase()
{
    ...
    for(int row = 1, col = 1; col <= n; ++col)
    {
        ...
        int _nr = inv(a[row][col]);
        for(int i = col; i <= n + 1; ++i)
            a[row][i] = mul(a[row][i], _nr);
    ...
    for(int i = 1; i <= n; ++i)
        if(where[i] != -1)
            ans[i] = mul(a[where[i]][n + 1], inv(a[where[i]][i]));
