...
const ll Inf = 1e18;
bool testCase = false;
const ll N = 109, V = 509, A = 26, LG = 31, Mod = 1e9 + 7;
...
void solve_testcase()
{
    ...
    FOR(i, 0, n)FOR(j, 0, V - 1)FOR(k, 0, V - 1)
        dp[i][j][k] = Inf;
    dp[0][0][0] = 0;
    ...
    FOR(i, 1, n)
    {
        if(a[i] == 1)
        {
            FOR(j, 0, V - 1)
                FOR(k, 0, V - 1)
                {
                    if(j >= b[i])dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j - b[i]][k]);
                    if(k >= b[i])dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j][k - b[i]] + 1);
        ...
        else if(a[i] == 2)
        {
            FOR(j, 0, V - 1)
                FOR(k, 0, V - 1)
                {
                    if(j >= b[i])dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j - b[i]][k] + 1);
                    if(k >= b[i])dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j][k - b[i]]);
                    dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j][k] + 1);
                }
        }
        else
