...
///cand adaug in ds verfic cate nr trec prin punctul x
...
int get_idx(int val)
{
    return distance(norma.begin(), lower_bound(norma.begin(), norma.end(), val)) + 1;
}
...
void solve_testcase()
{
    ...
    ///cele care int prin stanga
    sort(v + 1, v + n + 1, [](info a, info b){return a.l < b.l;});

    multiset<int> st;
    for(int i = 1; i <= n; ++i)
    {
        while(!st.empty() && *st.begin() <= v[i].l)st.erase(st.begin());

        int j = i;st.insert(v[i].r);
        while(j < n && v[i].l == v[j + 1].l)st.insert(v[j + 1].r), ++j;


        for(int k = i; k <= j; ++k)
        {
            st.erase(st.find(v[k].r));
            ans[v[k].ind] += distance(st.upper_bound(v[k].l + v[k].k - 1), st.end());
            st.insert(v[k].r);
        }
    ...
    ///cele care intra prin dreapta
    sort(v + 1, v + n + 1, [](info a, info b){return a.r > b.r;});

    multiset<int, greater<int>> stn;
    for(int i = 1; i <= n; ++i)
    {
        while(!stn.empty() && *stn.begin() >= v[i].r)stn.erase(stn.begin());

        int j = i;stn.insert(v[i].l);
        while(j < n && v[i].r == v[j + 1].r)stn.insert(v[j + 1].l), ++j;


        for(int k = i; k <= j; ++k)
        {
            stn.erase(stn.find(v[k].l));
            ans[v[k].ind] += distance(stn.upper_bound(v[k].r - v[k].k + 1), stn.end());
            stn.insert(v[k].l);
        }
