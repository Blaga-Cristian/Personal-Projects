...
intm PtLg(intm base, int exp)
{
    if(exp < 0)return PtLg(PtLg(base, -exp), Mod - 2);
...
int find(int x){return t[x] == 0 ? x : find(t[x]);}

intm get_val(int x)
{
    if(x == 0)return 0;
    return mars[x] + get_val(t[x]);
}

void unite(int a, int b, intm c)
{
    ...
    intm ans_a = get_val(a), ans_b = get_val(b);
    mars[rb] = mars[rb] + c * PtLg(10, n - sz[rb] - 1) + ans_a * PtLg(10, - sz[rb]);
    mars[ra] = mars[ra] + c * PtLg(10, n - sz[ra] - 1) + ans_b * PtLg(10, - sz[ra]) - mars[rb];
    t[ra] = rb;
    sz[rb] += sz[ra];
}
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)sz[i] = 1;
    sort(all(edges));
    for(auto e : edges)unite(e.a, e.b, e.c);

    FOR(i, 1, n)cout << get_val(i).v << '\n';
}
