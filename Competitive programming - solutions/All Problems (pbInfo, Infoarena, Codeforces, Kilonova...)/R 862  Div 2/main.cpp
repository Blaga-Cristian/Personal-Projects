...
int calc(int nod)
{
    ...
    return min(Inf, ds[nod] + ciclu + max(dt[a], dt[b]));
}
...
void solve_testcase()
{
    ...
    FOR(i, 2, n)
    {
        ...
        else if(cand == val && ds[i] < ds[nod])nod = i;
    }
    ...
    else
    {
        ...
        vi ans;
        ans.pb(nod);
        for(int x = t[nod]; x != nod; x = t[x])ans.pb(x);

        cout << (int)path.size() + (int)ans.size() - 1 << '\n';
