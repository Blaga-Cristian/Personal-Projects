...
bool testCase = true;
const ll N = 3e5 + 9, LG = 20, Mod = 1e9 + 7;

ll n, a[N], b[N], la[N], lb[N], ra[N], rb[N];
...
//gp_hash_table<pii, int, custom_hash_pair> cah;
int gcd(int a, int b)
{
    if(!a)return b;
    if(!b)return a;
//    if(cah[{a, b}])return cah[{a, b}];
...
void solve_testcase()
{
    ...
    FOR(i, 2, n)
    {
        if(ra[i] != ra[i - 1])lsta.push_back({i - 1, ra[i - 1]});
        if(rb[i] != rb[i - 1])lstb.push_back({i - 1, rb[i - 1]});
    }
    lsta.push_back({n, ra[n]});
    lstb.push_back({n, rb[n]});
    ...
    FORR(i, n, 1)
    {
        lst1.push_front({i, a[i]});
        for(auto j = lst1.begin(); j != lst1.end(); ++j)
        {
            auto k = next(j);
            k -> ss = gcd(j -> ss, a[k -> ff]);
            for(; k != lst1.end() && k -> ss == j -> ss; ++k)
            {
                k = lst1.erase(k);
                if(k == lst1.end())break;
                k -> ss = gcd(j -> ss, a[k -> ff]);
        ...
        lst2.push_front({i, b[i]});
        for(auto j = lst2.begin(); j != lst2.end(); ++j)
        {
            auto k = next(j);
            k -> ss = gcd(j -> ss, b[k -> ff]);
            for(; k != lst2.end() && k -> ss == j -> ss; ++k)
            {
                k = lst2.erase(k);
                if(k == lst2.end())break;
                k -> ss = gcd(j -> ss, b[k -> ff]);
        ...
        list<pii> e;
        e.merge(list<pii>() = lst1);
        e.merge(list<pii>() = lst2);
        e.merge(list<pii>() = lsta);
        e.merge(list<pii>() = lstb);
        ...
        for(auto it = e.begin(); it != e.end(); ++it)
        {
            ...
            while(next(it) != e.end() && next(it) -> ff == p)++ it;

            int p1;
            if(next(it) == e.end())p1 = n + 1;
            else p1 = next(it) -> ff;

            pii cand(gcd(la[i - 1], gcd(g2, ra[p + 1])) +
                     gcd(lb[i - 1], gcd(g1, rb[p + 1])), p1 - p);

            if(cand.ss != 0)ans = ans + cand;
        }
    }
