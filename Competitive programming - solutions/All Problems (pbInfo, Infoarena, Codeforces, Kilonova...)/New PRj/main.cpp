...
void solve_testcase()
{
    ...
    int nr = max(1LL, (k + m - 1) / m);

    multiset<pii> mst;
    for(int i = 1; i <= n; ++i)mst.insert({a[i], i});

    vi v;
    for(auto i : mst)if(v.size() < nr)v.pb(i.ss);///verific care are stack-ul incomplet
    sort(all(v));
    ...
    else
    {
        int init = ans;
        for(int i = 0; i < v.size(); ++i)
//            ans = min(ans, init - (m - r) * a[v[i]] - (m - r) * m * ((int)v.size() - i - 1));
            ans = min(ans, init - m * (a[v[i]] + i * m) - m * m * ((int)v.size() - i - 1)
                         + r * (a[v[i]] + i * m) + r * m * ((int)v.size() - i - 1));
