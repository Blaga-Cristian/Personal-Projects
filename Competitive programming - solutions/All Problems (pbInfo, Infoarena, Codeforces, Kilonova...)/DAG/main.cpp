...
int n, c[N], dp[N][N];///not quadangle
///dp[i][j] = costul minim pentru a gasi orice element din i...j
...
void solve_testcase()
{
    ...
    for(int sz = 2; sz <= n; ++sz)
    {
        ...
        {
            while(p < i)++p;
            while(p > i && max(dp[i][p - 1], dp[p + 1][j]) + c[p] > max(dp[i][p - 2], dp[p][j]) + c[p - 1])-- p;
            while(p < j && max(dp[i][p - 1], dp[p + 1][j]) + c[p] > max(dp[i][p], dp[p + 2][j]) + c[p + 1])++ p;
            dp[i][j] = dp[i][p - 1] + dp[p + 1][j] + c[p];
        }
    }
