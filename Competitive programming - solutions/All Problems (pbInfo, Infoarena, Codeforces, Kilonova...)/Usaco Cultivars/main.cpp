...
int get_ind(int val)
{
    return distance(normalize.begin(), lower_bound(all(normalize), val)) + 1;
}
...
{
    cin >> n;
    FOR(i, 1, n)cin >> v[i].l >> v[i].r >> v[i].k;

    FOR(i, 1, n)normalize.pb({v[i].l});
    sort(all(normalize));

    FOR(i, 1, n)Update(get_ind(v[i].))
}
