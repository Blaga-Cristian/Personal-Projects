...
//gp_hash_table<int, gp_hash_table<int, int>> ds;
//cc_hash_table<int, cc_hash_table<int, int>> ds;
unordered_map<int, unordered_map<int, int>> ds;
///mp[take] = give;

vpi f;

void Query(int p = 0, int div = 1)
{
    if(p == f.size())
    {
    }
...
void solve_testcase()
{
    ...
    {
        int g = gcd(p[i], i);
        int a = p[i] / g, b = i / g;///prea incet, incearca sa interschimbi
