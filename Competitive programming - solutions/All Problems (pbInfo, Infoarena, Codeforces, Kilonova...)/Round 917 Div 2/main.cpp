...
void solve_testcase()
{
    ...
    for(int i = 0; i < n; ++i)
    {
        for(int d = 0; d <= min(20, k - 1); ++d)
        {
            ll left = p[i] * (1ll << d);
            if(left <= 2 * n - 1)
            {
                ans += ll(k - d) * (ll)ft1.sum(left, 2 * n - 1);
                ans %= Mod;
            }
        }
    ...
    for(int i = n - 1; i >= 0; --i)
    {
        for(int d = 1; d <= min(19, k - 1); ++d)
        {
            ll right = min(2 * n - 1ll, p[i] * (1ll << d));
            ans += (ll)(k - d) * (ll)ft2.sum(0, right);
            ans %= Mod;
        }
