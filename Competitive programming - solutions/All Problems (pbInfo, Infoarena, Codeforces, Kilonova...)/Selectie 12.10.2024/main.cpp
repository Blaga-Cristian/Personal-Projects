...
bool testCase = false;
const ll N = 2009, LG = 31, Mod = 1e9 + 7;
...
struct edge
{
    ...
    edge(int from = 0, int to = 0, int cost = 0, int cap = 0, int flow = 0) :
        from(from), to(to), cost(cost), cap(cap), flow(flow) {}
};
vector<edge> edges;

void add_edge(int x, int y, int c, int cap)
{
    G[x].pb(edges.size());
    edges.pb({x, y, c, cap, 0});
    G[y].pb(edges.size());
    edges.pb({y, x, -c, 0, 0});
}
...
bool Dijkstra()
{
    ...
    while(!q.empty())
    {
        ...
        {
            auto& e = edges[i];
            if(e.cap - e.flow > 0 && d[e.to] > d[x] + e.cost)
            {
                d[e.to] = d[x] + e.cost;
                t[e.to] = i;
                q.push({d[e.to], e.to});
...
int Max_Flow()
{
    ...
    {
        int flow = Inf;
        for(int x = 2 * n + 1; x; x = edges[t[x]].from)
...
//    v[x] = true;
//    ans.pb(x);
//    if(R[R[x]] && !v[R[R[x]]])Dfs(R[R[x]]);
//}

void solve_testcase()
{
    cin >> n;
    cin >> a;///unde trebuie sa fie
    cin >> b;///profesorii
    ...
    FOR(i, 0, n - 1)
        FOR(j, 0, n - 1)
            ...
            {
                if(i == j);
                else add_edge(i + 1, j + n + 1, 1, 1);
            }

    FOR(i, 1, n)add_edge(0, i, 0, 1);
    FOR(i, n + 1, 2 * n)add_edge(i, 2 * n + 1, 0, 1);



    v.reset();
    for(auto e : edges)
        if(e.from >= 1 && e.from <= n && e.to >= n + 1 && e.to <= 2 * n
           && e.flow > 0)
        {
            R[e.from] = e.to - n;
            L[e.to - n] = e.from;
    ...
    FOR(i, 1, n)
    {
        ...
        ans.pb("DRIVE " + to_string(x) + "\n");
        ans.pb("PICKUP\n");
        ans.pb("DRIVE " + to_string(R[x]) + "\n");
        ans.pb("DROPOFF\n");
        v[x] = v[R[x]] = true;
        x = R[x];
        while(R[x] && !v[R[x]])
