...
ll aib[N], cnt[N];
void UpdateV(int p, ll v)
{
    for(; p <= n; p += p & -p)
        aib[p] += v;
}
void UpdateC(int p, ll v)
{
    for(; p <= n; p += p & -p)
        cnt[p] += v;
}
ll Find(ll x)
{
    ll sum = 0, c = 0, pos = 0;
    for(int b = 18; b >= 0; --b)
        if(pos + (1 << b) <= n && sum + aib[pos + (1 << b)] < x)
        {
            sum += aib[pos + (1 << b)];
            c += cnt[pos + (1 << b)];
            pos += (1 << b);
...
void solve_testcase()
{
    ...
    FOR(i, 1, n)UpdateV(inv[i], a[i] - b[i]);
    FOR(i, 1, n)UpdateC(inv[i], 1);
